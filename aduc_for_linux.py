...
class SecurityAclEditor(QWidget):
    ...
    def on_principal_changed(self) -> None:
        ...
        mapped_bits = 0
        for _perm_name, bit in self.PERMISSIONS:
            if bit > 0:
                mapped_bits |= bit
        self._loading_permissions = True
        for row, (_perm_name, bit) in enumerate(self.PERMISSIONS):
            if bit == 0:
                self.permissions_table.item(row, 1).setCheckState(Qt.Checked if (allow_mask & ~mapped_bits) else Qt.Unchecked)
                self.permissions_table.item(row, 2).setCheckState(Qt.Checked if (deny_mask & ~mapped_bits) else Qt.Unchecked)
            else:
